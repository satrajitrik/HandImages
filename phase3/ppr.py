...
class PageRank(object):
    ...
    def label_images(self):
        ...
        for i in range(len(self.image_vectors)):
            ...
            self.labels[self.image_vectors[i]['image_id']] = self.image_vectors[i]['label']
    
        for i in range(len(self.unlabelled_image_vectors)):
            self.unlabelled_image_names.append(self.unlabelled_image_vectors[i]['image_id'])
        ...
        for i in range(self.node_count):
            for j in range(i + 1, self.node_count):
                # Compute Image similarity
                similarity = functions.calculate_similarity(self.all_image_vectors[i]['vector'],
    ...
    def perform_random_walk(self, query_images, label=False):
        ...
        """
        try:
            query_image_positions = [self.all_image_names.index(x) for x in query_images]
        except ValueError:
