...
class Node:
    ...
    #
    #     n_data= y.size
    #     #if less than one data is present then stop spliting.
    #     if(n_data <= 1 ):
    #         return None, None
    #
    #     #calculate the current class probability and gini_index
    #     parent = [np.sum(y == c) for c in range(self.n_classes_)]
    #     parent_gini_index = 1.0 - [(x/n_data) * (x/n_data) for x in parent]
    #
    #
    #     '''
    #         for finding best_feature with best_threshold we loop through all features.
    #     '''
    #     best_feature = None
    #     best_threshold = None
    #     best_gini_index = parent_gini_index
    #
    #     for idx in range(self.n_features_):
    #         thresholds, classes = zip(*sorted(zip(X[:, idx], y)))
    #
    #         left= [0] * self.n_classes_
    #         right = parent.copy()
    #
    #         for i in range(1, n_data): # split position
    #             c = classes[i-1]
    #             left[c] += 1
    #             right[c] -= 1
    #
    #             gini_left = 1.0 - sum(
    #                 (left[x] / i) ** 2 for x in range(self.n_classes_)
    #             )
    #             gini_right = 1.0 - sum(
    #                 (right[x] / (n_data - i)) ** 2 for x in range(self.n_classes_)
    #             )
    #
    #             gini = ((i/n_data) * gini_left) + (((n_data - i)/n_data) * gini_right)
    #             if thresholds[i] == thresholds[i - 1]:
    #                 continue
    #
    #             if gini < best_gini:
